...
const SunglassSchema = new mongoose.Schema({
  id: { type: Number, unique: true }, // Sequential ID for sunglasses
  ...
  lens_material: { type: String, required: true },
  prescription_possible: { type: String, enum: ['Yes', 'No'], required: true },
  copy_type: { type: String, required: true },
...
SunglassSchema.pre('save', async function (next) {
  if (!this.id) {
    try {
      // Find and increment the counter
      const counter = await Counter.findOneAndUpdate(
        { modelName: 'Sunglass' }, // Use `_id` to match the Counter schema
...
SunglassSchema.post('findOneAndUpdate', async function (doc, next) {
  if (doc) {
...
// Sunglass model
const Sunglass = mongoose.models.Sunglass || mongoose.model('Sunglass', SunglassSchema);
