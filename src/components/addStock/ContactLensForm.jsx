                    ...
                    {lens.colors.map((color, colorIndex) => (
                        <div key={colorIndex} className="mb-4 p-4 border rounded">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label className="block mb-1">Color Tags (comma-separated)</label>
                                    <input
                                        type="text"
                                        value={color.tags ? color.tags.join(', ') : ''}
                                        onChange={(e) => {
                                            const tags = e.target.value.split(',').map(tag => tag.trim());
                                            handleColorChange(colorIndex, 'tags', tags);
                                        }}
                                        className="w-full p-2 border rounded"
                                    />
                                </div>
